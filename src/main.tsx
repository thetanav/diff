...
const DiffFileView = memo(function DiffFileView({
  ...
  return (
    <section className="file">
      ...
      {open ? (
        <div className="diff" aria-label={`${path} diff`}>
          {file.hunks.map((hunk, hunkIndex) => (
            <div className="hunk" key={`${hunk.header}-${hunkIndex}`}>
          ...
          {file.omittedLines > 0 ? (
            <div className="omitted">{file.omittedLines} lines hidden by the server cap</div>
          ) : null}
        </div>
      ) : null}
    </section>
...
function App() {
  ...
  async function loadDiff(event: FormEvent) {
    ...
    try {
      const response = await fetch(`/api/pr-diff?pr=${encodeURIComponent(pr)}`);
    ...
    <main>
      <header className="topbar">
        <div>
          ...
          <p>Fast, low-detail GitHub pull request diffs.</p>
        </div>
        <form onSubmit={loadDiff}>
          ...
          />
          <button disabled={loading || pr.trim().length === 0}>
      ...
      {payload ? (
        <>
          ...
          {payload.omittedFiles > 0 ? (
            <div className="notice">{payload.omittedFiles} files hidden by the server cap</div>
          ) : null}

          <div className="files">
            {files.map((file, index) => (
              <DiffFileView file={file} defaultOpen={index < 2} key={`${file.oldPath}-${file.newPath}`} />
            ))}
          </div>
        </>
      ) : (
        <section className="empty">Enter a GitHub pull request to render a compact diff.</section>
      )}
    </main>
  );
}
